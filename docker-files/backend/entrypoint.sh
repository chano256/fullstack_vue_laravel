...
if [ "$1" = 'mysqld' -a -z "$wantHelp" ]; then
	...
	if [ ! -d "$DATADIR/mysql" ]; then
		...
		echo 'SHOW parameters done...'
		if [ "$i" = 0 ]; then
			echo >&2 'MySQL init process failed.'
			exit 1
		fi

		echo 'TEST mysql bug https://bugs.mysql.com/bug.php?id=20545...'
		echo "$MYSQL_INITDB_SKIP_TZINFO"
		echo "${mysql[@]}"
		if [ -z "$MYSQL_INITDB_SKIP_TZINFO" ]; then
			echo 'Found mysql bug https://bugs.mysql.com/bug.php?id=20545...'
			# sed is for https://bugs.mysql.com/bug.php?id=20545
			mysql_tzinfo_to_sql /usr/share/zoneinfo | sed 's/Local time zone must be set--see zic manual page/FCTY/' | "${mysql[@]}" mysql
		fi

		echo 'CHECKING IF USE RANDON PASSWORD SELECTED...'
		if [ ! -z "$MYSQL_RANDOM_ROOT_PASSWORD" ]; then
			export MYSQL_ROOT_PASSWORD="$(pwgen -1 32)"
			echo "GENERATED ROOT PASSWORD: $MYSQL_ROOT_PASSWORD"
		fi

		echo 'CHECKING IF USE MYSQL ROOT HOST SET...'
		rootCreate=
		...
		echo 'CHECKING IF MYSQL ROOT PASSWORD SET...'
		if [ ! -z "$MYSQL_ROOT_PASSWORD" ]; then
			mysql+=( -p"${MYSQL_ROOT_PASSWORD}" )
		fi

		echo 'CHECKING IF MYSQL DATABASE SET...'
		...
		echo 'CHECKING IF MYSQL USER AND PASSWORD SET...'
